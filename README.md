...
finding. For private repos without GitHub Advanced Security, findings can instead
(or additionally) go to [Balam](https://github.com/colibrisec/balam).
...
### Private repo, no GitHub Advanced Security

Code scanning (`sarif: true`) needs GHAS on a private repo. If you don't have it,
either turn `sarif` off or send findings to Balam instead:
...
Mint `balam-token` as a personal API token in Balam (Balam's own UI/API — not covered
here) and store it as a repo/org secret.
...
| `sbom` | `false` | CycloneDX SBOM (`ojo -f sbom`), published as a workflow artifact only — not uploaded to Balam or code scanning |
...
| `sarif` | `true` | Upload SARIF to GitHub code scanning (needs `security-events: write`; private repos need GHAS). `false` leaves the JSON report (already generated for the summary/issues/Balam) as the only per-scan output. |
...
| `tracker` | `none` | Also upload findings to a tracker: `balam` or `none` |
| `balam-url` | | Required when `tracker: balam` |
| `balam-token` | | Required when `tracker: balam`; pass via a secret |
| `product-name` / `product-id` | | One of these required for Balam (name auto-creates the product) |
| `engagement-name` / `engagement-id` | | One of these required for Balam (name auto-creates the engagement) |
| `product-type-name` | | Optional, Balam only |
| `test-title` | | Optional, Balam only |
